...
from .models import Book, Author, BookInstance, Genre, Category, Item, WishList
...
def index(request):
    ...
    categorie = []
    i = 0
    while i < len(num_categories):
        categorie.append(num_categories[i])
        i = i + 1
    # str1 = " "
    #
    # # traverse in the string
    # for ele in categorie:
    #     str1 += "✿♥‿♥✿"+ ele
    ...
    return render(
        ...
        context={'num_books': num_books, 'num_instances': num_instances,
                 ...
                 'num_visits': num_visits, "num_categories": num_categories, "categorie": categorie},
    )
...
class WishListView(LoginRequiredMixin, generic.ListView):
    """Generic class-based view listing books on loan to current user."""
    model = WishList
    template_name = 'catalog/wishlist_list_user.html'
    paginate_by = 10

    def get_queryset(self):
        return WishList.objects.filter(wisher=self.request.user)
...
def add_to_wishlist(request, pk):
    ...
    return render(request, 'catalog/wishlist_add_item.html', context)
